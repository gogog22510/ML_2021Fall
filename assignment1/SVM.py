...
class SupportVectorClassifier(base.Experiment):
    ...
    def run(self):
        ...
        for kernel in kernels:
            self.log(f"Start training with kernel = {kernel}...")
            # Train a SVM
            model = svm.SVC(
        ...
        self.log(f"Testing Metrics ({best_kernel}):")
    ...
    def plot(self, data):
        ...
        for k in ['train', 'validation']:
            ...
            plt.subplot(1, 2, 1)
            plt.plot(kernels, [met[k]['accuracy'] for met in metrics])
            plt.xlabel("Kernel")
            plt.ylabel(f"Accuracy ({k})")

            plt.subplot(1, 2, 2)
            plt.plot(kernels, [met[k]['mse'] for met in metrics])
            plt.xlabel("Kernel")
            plt.ylabel(f"MSE ({k})")

            plt.savefig(f"plot/SVM_{k}.png")
